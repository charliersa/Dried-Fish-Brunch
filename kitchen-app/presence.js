...
(function () {
  ...
  /* ---------------- 顧客端：送心跳 ---------------- */
  // always=true（後台頁用）：不做閒置判斷，頁面活著就持續回報——
  // 用來遠端判斷廚房／收銀頁的 JS 有沒有被系統凍結
  function beat(page, always) {
    var d = db();
    if (!d) return;
    var ref;
    ...
    function active() {
      return always || Date.now() - lastActive < IDLE_MS;
    }
  ...
  if (page) beat(page, !!(tag && tag.hasAttribute('data-always')));
})();
