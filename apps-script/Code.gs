/**
 * 小魚乾早午餐 —— 每日報表寫入 Google Sheet
 *
 * 這份程式碼放在 Google Apps Script 專案裡（script.google.com），不是網站的一部分。
 * 收進 repo 只是為了版控與對照：Google 那邊改了，這裡也要跟著更新。
 *
 * 後台「📊 同步到 Google Sheet」會 POST 一包 JSON 過來，格式：
 *   {
 *     date: '2026-08-15',
 *     store: '小魚乾早午餐',
 *     summary: { count, paidCount, unpaidCount, revenue,
 *                cashCount, cashSum, mobileCount, mobileSum },
 *     orders: [{ no, time, typeTable, status, pay, total }],
 *     items:  [{ name, qty, rev }]
 *   }
 *
 * 一定要回傳 JSON。舊版沒有回傳值，瀏覽器只好用 no-cors 送出，
...
function doPost(e) {
  try {
    ...
    const name = String(p.date || '').trim();
    if (!name) return json_({ ok: false, error: '沒有收到日期' });

    const ss = SpreadsheetApp.openById(SHEET_ID);
    // 同一天重跑要整份覆蓋，不然分頁會越疊越多份
    let sh = ss.getSheetByName(name);
    if (sh) sh.clear(); else sh = ss.insertSheet(name);

    const s = p.summary || {};
    const rows = [];
    rows.push([(p.store || '') + ' 每日報表', name]);
    ...
    (p.orders || []).forEach(function (o) {
      // 單號與時間強制當文字，否則 6:32 會被 Sheets 吃成時間、C01 之類也可能被轉型
      rows.push(["'" + o.no, "'" + o.time, o.typeTable, o.status, o.pay, o.total]);
    });
    ...
    return json_({ ok: true, sheet: name, rows: (p.orders || []).length, url: ss.getUrl() });
  } catch (err) {
    return json_({ ok: false, error: String(err) });
  }
}
